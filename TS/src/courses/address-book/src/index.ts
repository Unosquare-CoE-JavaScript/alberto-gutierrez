...
type group = { [k: string]: any };
interface addressContact {
  firstName?: undefinedString;
  lastName?: undefinedString;
  middleName?: undefinedString;
  salutation?: undefinedString;
  email?: undefinedString;
  addresses: group;
  phones: { [k: string]: number | string };
}
export class AddressBook {
  contacts: Array<addressContact> = [];
  addContact(contact: addressContact) {
...
function getFullName(contact: addressContact) {
  return [contact.firstName, contact.middleName, contact.lastName]
    .filter(Boolean)
    .join(" ");
}

export function getVcardText(contact: addressContact, date = new Date()) {
  const parts = [
    "BEGIN:VCARD",
    "VERSION:2.1",
    ...
    ...Object.keys(contact.phones).map(
      (phName: string) =>
        `TEL;${phName.toUpperCase()};VOICE:${contact.phones[phName]}`,
    ),
